 ...
 * implementation. For example, if we have a REST API available then the developers of
 * the application could easily switch to another type of database (e.g. cache, file storage, NoSQL)
 * and change the repository type based on client requirements and needs. This means that
 * our integration tests would still work for all underlying repositories as we would be testing the APIs
 * end to end. For cases where we don't have the API we need to use the database directly which means that
 ...
 * should be minimal, for example just for database cleanup purposes (e.g. deleteAll)
 * and in a perfect world there should be none as we can teardown the environment,
 * e.g. docker container, instead of deleteAll from DB.
 */
